        ...
        for line in block.get("lines", []):
            for span in line.get("spans", []):
                ...
                if r >= VERY_BRIGHT_TEXT_THRESHOLD and g >= VERY_BRIGHT_TEXT_THRESHOLD and b >= VERY_BRIGHT_TEXT_THRESHOLD:
                    bright_span_count += 1
    return {"span_count": span_count, "bright_span_count": bright_span_count, "text_chars": text_chars, "bright_ratio": round(bright_span_count / span_count, 3) if span_count else 0.0}
...
def _image_meta(doc, xref: int) -> Dict:
    ...
    return {"xref": xref, "ok": True, "width": info.get("width", 0), "height": info.get("height", 0), "size": len(info.get("image", b""))}
...
def analyze_pdf_pages(pdf_path: str) -> Dict:
    ...
    for idx, page in enumerate(doc):
        ...
        pages.append({"page_number": idx + 1, "text_chars": t["text_chars"], "span_count": t["span_count"], "bright_ratio": t["bright_ratio"], "image_count": len(image_xrefs), "large_image_count": large_images, "image_bytes": image_bytes, "remove_safe": bool(t["text_chars"] > 0 and t["bright_ratio"] < 0.08)})
            ...
            for xref in list(dict.fromkeys([img[0] for img in page.get_images(full=True)])):
                try:
                    ...
                    if not info or not info.get("image"):
                        continue
                    new_bytes = _downsample_and_jpeg(info["image"], quality=JPEG_QUALITY_BALANCED if mode != "text_priority" else JPEG_QUALITY_AGGRESSIVE, max_side=DOWNSAMPLE_MAX_SIDE)
                    if len(new_bytes) < len(info["image"]):
                        _replace_image(doc, xref, new_bytes)
                except Exception:
                    continue
        page_actions.append({"page": page_info["page_number"], "action": action})
    try:
        doc.save(output_path, garbage=4, clean=True, deflate=True, use_objstms=1)
    except TypeError:
        doc.save(output_path, garbage=4, clean=True, deflate=True)
    optimized_size = os.path.getsize(output_path)
    return {"original_bytes": original_size, "optimized_bytes": optimized_size, "reduction_bytes": max(0, original_size - optimized_size), "reduction_ratio": round((1 - optimized_size / original_size) * 100, 2) if original_size else 0.0, "mode": mode, "has_text_layer": analysis["has_text_layer"], "page_actions": page_actions, "page_analysis": analysis["pages"]}
